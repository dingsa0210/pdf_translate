...
流程: 扫描PDF→渲染→分块OCR→合并结果→翻译→白底擦除→左对齐/旋转回填→重构PDF
"""
...
def translate_with_llm(text_items: list) -> list:
    ...
    重点：对多行/编号块保留行结构、编号和顺序，避免后续回填时重排。
    """
    try:
    ...
    dict_sample = "\n".join([f'  "{cn}" → "{en}"' for cn, en in list(ENGINEERING_DICT.items())[:30]])
...
6. 短语翻译尽量控制在3个单词以内
7. 如果原文本含有多行、编号列表或缩进，请严格保留原来的行数、顺序、编号和缩进，不要合并行，不要重排，不要把下一行挤到上一行后面
8. 对于“技术要求”这类结构化文本，优先保持每条要求独占一行
...
{dict_sample}"""
    ...
    for batch_idx in range(0, len(sorted_items), batch_size):
        ...
        items_str = "\n".join([f'{j + 1}. "{item[1]["text"]}"' for j, item in enumerate(batch)])
        user_prompt = f"""待翻译文本：
请逐条翻译，并保持每条原始结构。
...
def _is_structured_text(source_text, translated_text):
    """Detect numbered lists / multi-line blocks that should preserve line structure."""
    if "\n" in source_text or "\n" in translated_text:
        return True
    ...
    return any(re.search(pat, combined) for pat in structured_markers)
...
def _fit_text_to_box(draw, text, font_path, box_w, box_h, max_font_size, structured=False):
    """Shrink text to fit the target box.
    For structured content, preserve line breaks and avoid re-wrapping across lines.
    """
    min_font_size = 6
    max_font_size = max(min_font_size, max_font_size)
    ...
    for font_size in range(max_font_size, min_font_size - 1, -1):
        font = _load_font(font_path, font_size)
        spacing = max(0, int(font_size * 0.15))
        lines = source_lines[:] if structured else _wrap_text_to_width(draw, text, font, max(1, box_w))
        ...
        if max_line_w <= box_w and total_h <= box_h:
            return font, lines, spacing, max_line_w, total_h, line_heights
...
def _alpha_composite_at(base_rgba, layer_rgba, left, top):
    """Paste an RGBA layer onto another RGBA image with clipping."""
    base_w, base_h = base_rgba.size
    layer_w, layer_h = layer_rgba.size

    x1 = max(0, left)
    y1 = max(0, top)
    x2 = min(base_w, left + layer_w)
    y2 = min(base_h, top + layer_h)
    if x2 <= x1 or y2 <= y1:
        return
...
def _render_text_item_layer(item, font_path, text_color):
    """Render a single OCR item as a transparent layer, optionally rotated."""
    x1, y1, x2, y2 = item["bbox"]
    box_w = max(1, x2 - x1)
    box_h = max(1, y2 - y1)
    ...
    structured = _is_structured_text(original, translated)

    pad = max(4, int(min(box_w, box_h) * 0.12))
    local_w = max(1, box_w + pad * 2)
    local_h = max(1, box_h + pad * 2)
    local = Image.new("RGBA", (local_w, local_h), (0, 0, 0, 0))
    d = ImageDraw.Draw(local)

    max_font_size = max(8, int(box_h * 0.82))
    font, lines, spacing, text_w, text_h, line_heights = _fit_text_to_box(
        d,
        translated,
        font_path,
        local_w - 2 * pad,
    ...
    for i, line in enumerate(lines):
        ...
        line_h = bbox[3] - bbox[1]
        d.text((tx, y_cursor), line, fill=text_color + (255,), font=font)
        y_cursor += line_h + (spacing if i < len(lines) - 1 else 0)
...
def inpaint_and_overlay(img_path, translated_items, output_img_path):
    """For scanned CAD PDFs only:
    1) wipe OCR text regions with a flat background color
    2) draw translated text left-aligned, preserving line structure
    3) rotate slanted text back to the original angle
    4) keep the rest of the raster page unchanged
    """
    img_bgr = cv2.imread(img_path)
    if img_bgr is None:
        raise FileNotFoundError(f"Unable to read image: {img_path}")
